...
class AgendaBewerkService {
  ...
  static String? kanItemBewerken({
    ...
    final bestaandeItems = List<AgendaItem>.from(itemsPerDag[datumKey] ?? []);

    bestaandeItems.removeWhere((item) => zelfdeItem(item, oudItem));
  ...
  static Map<String, List<AgendaItem>> bewerkItem({
    ...
    final items = List<AgendaItem>.from(kopie[datumKey] ?? []);

    final index = items.indexWhere((item) => zelfdeItem(item, oudItem));
