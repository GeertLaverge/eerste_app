// THIMACO-CONTROLE: VOORZETSCREEN-PAINTER-GESLOTEN-GELEIDERS-20260730-2005
...
class OpmetingVoorzetscreenPainter extends CustomPainter {
  const OpmetingVoorzetscreenPainter({required this.model});
  ...
  static const double _vrijeOnderruimteMm = 300.0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _tekenTitel(canvas, 'Vooraanzicht', Offset(voorZone.center.dx, 12));
    _tekenTitel(canvas, 'Zijaanzicht', Offset(zijZone.center.dx, 12));
  ...
  void _tekenVooraanzicht(Canvas canvas, Rect rect, double schaal) {
    final kastMm = model.kastmaat.millimeter.toDouble();
    final kastHoogte = math.max(12.0, kastMm * schaal);
    final geleiderBreedte = math.max(6.0, math.min(14.0, 28 * schaal));
    final onderlatHoogte = math.max(6.0, math.min(12.0, 22 * schaal));
    final vrijeOnderruimte = _vrijeOnderruimteMm * schaal;
    ...
    canvas.drawRect(
      doekRect,
      Paint()..color = _kleurUitHex(model.doekVoorzijdeHex),
    );
    _tekenDoekStructuur(canvas, doekRect);
    canvas.drawRect(doekRect, profielRand);

    final onderlat = Rect.fromLTRB(
      doekRect.left,
      doekRect.bottom,
      doekRect.right,
      doekRect.bottom + onderlatHoogte,
    );
    ...
    _tekenGeleiderZonderOnderlijn(
      canvas,
      linksGeleider,
      vulling: profielVulling,
      rand: profielRand,
    );
    _tekenGeleiderZonderOnderlijn(
      canvas,
  ...
  void _tekenZijaanzicht(
    ...
    final maat = model.kastmaat.millimeter.toDouble();
    final kastGrootte = maat * schaal;
    ...
    final kastPad = _kastProfielPad(kastRect, model.kastvorm);
    ...
    _tekenGeleiderZonderOnderlijn(
      canvas,
      geleiderRect,
      vulling: Paint()..color = _profielSchaduw,
      rand: rand,
    );
  ...
  Path _kastProfielPad(Rect rect, OpmetingVoorzetscreenKastvorm vorm) {
    switch (vorm) {
  ...
  void _tekenDoekStructuur(Canvas canvas, Rect rect) {
    final paint = Paint()
      ..color = Colors.white.withValues(alpha: 0.23)
      ..strokeWidth = 0.65;
    const stap = 9.0;
    for (double x = rect.left - rect.height; x < rect.right; x += stap) {
      canvas.drawLine(
  ...
  void _tekenGeleiderZonderOnderlijn(
    Canvas canvas,
    Rect rect, {
    required Paint vulling,
    required Paint rand,
  }) {
    canvas.drawRect(rect, vulling);
    canvas.drawRect(rect, rand);
  }
  ...
  void _tekenTitel(Canvas canvas, String tekst, Offset positie) {
    _tekenTekst(canvas, tekst, positie, fontSize: 11, vet: true);
  }
  ...
  @override
  bool shouldRepaint(covariant OpmetingVoorzetscreenPainter oldDelegate) {
