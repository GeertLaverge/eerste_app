// THIMACO-CONTROLE: TECHNISCHE-KEUZES-VERPLAATSKNOPPEN-EN-VEILIG-GROEP-WISSEN-20260914
// THIMACO-CONTROLE: TECHNISCHE-KEUZES-GROEPEN-FASE10-20260913
// THIMACO-CONTROLE: RAAM-TEKENPROGRAMMA-V8-UNIEKE-KETEN-CONTROLE-20260913
...
import 'opmeting_raam_technische_keuzes_ribbon_v10.dart';
...
class OpmetingRaamFormulierLayoutV10 extends StatelessWidget {
  const OpmetingRaamFormulierLayoutV10({
    super.key,
    ...
    required this.onNieuweTechnischeGroep,
    required this.onTechnischeGroepZichtbaarheidGewijzigd,
    required this.onTechnischeGroepVerwijderen,
    required this.toonTechnischeKeuzeVerplaatsKnoppen,
    required this.onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd,
  ...
  final Future<void> Function() onNieuweTechnischeGroep;
  final Future<void> Function(String groepId, bool zichtbaar)
  onTechnischeGroepZichtbaarheidGewijzigd;
  final Future<void> Function(String groepId) onTechnischeGroepVerwijderen;
  final bool toonTechnischeKeuzeVerplaatsKnoppen;
  final ValueChanged<bool> onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd;
  ...
  Widget _bouwProgrammaBovenbalk() {
    ...
    final technischeKeuzesRibbon = OpmetingRaamTechnischeKeuzesRibbonV10(
      ...
      toonVerplaatsKnoppen: toonTechnischeKeuzeVerplaatsKnoppen,
    ...
    return _OpmetingRaamProgrammaKop(
      ...
      technischeGroepen: technischeGroepen,
      onNieuweTechnischeGroep: onNieuweTechnischeGroep,
      onNieuweTechnischeKeuze: onMenuToevoegen,
      onTechnischeGroepZichtbaarheidGewijzigd:
          onTechnischeGroepZichtbaarheidGewijzigd,
      onTechnischeGroepVerwijderen: onTechnischeGroepVerwijderen,
      toonTechnischeKeuzeVerplaatsKnoppen:
          toonTechnischeKeuzeVerplaatsKnoppen,
      onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd:
          onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd,
...
class _OpmetingRaamProgrammaKop extends StatefulWidget {
  const _OpmetingRaamProgrammaKop({
    ...
    required this.technischeGroepen,
    required this.onNieuweTechnischeGroep,
    required this.onNieuweTechnischeKeuze,
    required this.onTechnischeGroepZichtbaarheidGewijzigd,
    required this.onTechnischeGroepVerwijderen,
    required this.toonTechnischeKeuzeVerplaatsKnoppen,
    required this.onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd,
  ...
  final List<OpmetingRaamTechnischeGroep> technischeGroepen;
  final Future<void> Function() onNieuweTechnischeGroep;
  final Future<void> Function() onNieuweTechnischeKeuze;
  final Future<void> Function(String groepId, bool zichtbaar)
  onTechnischeGroepZichtbaarheidGewijzigd;
  final Future<void> Function(String groepId) onTechnischeGroepVerwijderen;
  final bool toonTechnischeKeuzeVerplaatsKnoppen;
...
class _OpmetingRaamProgrammaKopState
        ...
        Container(
          ...
          child: Row(
            children: <Widget>[
              ...
              _bouwTechnischeKeuzesTabMetMenu(),
  ...
  Widget _bouwTechnischeKeuzesTabMetMenu() {
    final open =
        widget.openRibbon == OpmetingRaamProgrammaRibbon.technischeKeuzes;
    ...
    return Row(
      ...
      children: <Widget>[
        Tooltip(
          message: open ? 'Technische keuzes sluiten' : 'Technische keuzes openen',
          child: InkWell(
            onTap: () {
              _wisselRibbon(OpmetingRaamProgrammaRibbon.technischeKeuzes);
            },
        ...
        PopupMenuButton<String>(
          tooltip: 'Technische groepen beheren',
          ...
          onSelected: (waarde) {
            if (waarde == 'nieuwe_groep') {
              widget.onNieuweTechnischeGroep();
              return;
            }
            if (waarde == 'nieuwe_keuze') {
              widget.onNieuweTechnischeKeuze();
              return;
            }
            if (waarde == 'verplaatsknoppen') {
              widget.onToonTechnischeKeuzeVerplaatsKnoppenGewijzigd(
                !widget.toonTechnischeKeuzeVerplaatsKnoppen,
              );
              return;
            }
            if (waarde.startsWith('verwijder_groep:')) {
              final groepId = waarde.substring('verwijder_groep:'.length);
              widget.onTechnischeGroepVerwijderen(groepId);
              return;
            }
            if (waarde.startsWith('groep:')) {
              final groepId = waarde.substring('groep:'.length);
              OpmetingRaamTechnischeGroep? groep;
              for (final kandidaat in widget.technischeGroepen) {
              ...
              if (groep != null) {
                widget.onTechnischeGroepZichtbaarheidGewijzigd(
                  groep.id,
                  !groep.zichtbaar,
                );
              }
          ...
          itemBuilder: (context) => <PopupMenuEntry<String>>[
            ...
            if (groepen.isEmpty)
              const PopupMenuItem<String>(
                ...
                child: Text(
                  ...
                  style: TextStyle(color: ThimacoKleuren.tekstGrijs),
                ),
              ),
            ...groepen.map(
              (groep) => CheckedPopupMenuItem<String>(
                value: 'groep:${groep.id}',
                checked: groep.zichtbaar,
                child: Text(
                  groep.naam,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
            ...
            CheckedPopupMenuItem<String>(
              value: 'verplaatsknoppen',
              checked: widget.toonTechnischeKeuzeVerplaatsKnoppen,
              child: const Text('Verplaatsknoppen tonen'),
            ),
            if (groepen.isNotEmpty) ...<PopupMenuEntry<String>>[
              const PopupMenuDivider(),
              const PopupMenuItem<String>(
                enabled: false,
                child: Text(
              ...
              ...groepen.map(
                (groep) => PopupMenuItem<String>(
                  value: 'verwijder_groep:${groep.id}',
                  child: Row(
                    children: <Widget>[
                      const Icon(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Container(
                  ...
                  height: 1.5,
                  decoration: BoxDecoration(
                    color: ThimacoKleuren.oranje.withValues(alpha: 0.68),
                    borderRadius: BorderRadius.circular(99),
                  ),
                ),
              ],
