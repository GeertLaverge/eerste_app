// THIMACO-CONTROLE: PRIJS-VOOR-ALLE-POSITIES-INSTELLINGEN-PAGINA-20260815
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

import '../../../helpers/app_storage.dart';
import '../../../helpers/offerte/prijzen/offerte_artikel_prijs_data_model.dart';
import '../../../helpers/offerte/prijzen/offerte_prijs_voor_alle_posities_template_model.dart';

class OffertePrijsVoorAllePositiesPagina extends StatefulWidget {
  const OffertePrijsVoorAllePositiesPagina({super.key});

  @override
  State<OffertePrijsVoorAllePositiesPagina> createState() {
    return _OffertePrijsVoorAllePositiesPaginaState();
  }
}

class _OffertePrijsVoorAllePositiesPaginaState
    extends State<OffertePrijsVoorAllePositiesPagina> {
  ...
  List<OffertePrijsVoorAllePositiesTemplateModel> _templates =
      <OffertePrijsVoorAllePositiesTemplateModel>[];
  bool _laden = true;
  bool _opslaan = false;
  String? _foutmelding;
  ...
  static int _vergelijkTemplates(
    OffertePrijsVoorAllePositiesTemplateModel eerste,
    OffertePrijsVoorAllePositiesTemplateModel tweede,
  ) {
  ...
  Future<bool> _bewaar(
    List<OffertePrijsVoorAllePositiesTemplateModel> nieuweTemplates,
  ) async {
    ...
    try {
      await AppStorage.bewaarOffertePrijsVoorAllePositiesTemplates(
        nieuweTemplates,
      );
      if (!mounted) return true;

      setState(() {
        _templates = List<OffertePrijsVoorAllePositiesTemplateModel>.from(
          nieuweTemplates,
  ...
  Future<void> _voegToe() async {
    ...
    final nieuw = await _toonTemplateDialog(
      begin: OffertePrijsVoorAllePositiesTemplateModel(
        id: 'prijs_alle_posities_${DateTime.now().microsecondsSinceEpoch}',
        omschrijving: '',
    ...
    await _bewaar(<OffertePrijsVoorAllePositiesTemplateModel>[
      ..._templates,
      nieuw,
    ]);
  }
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Text(
            ...
            'Reeds gebruikte regels in bestaande offertes blijven ongewijzigd.',
          ),
          actions: <Widget>[
            TextButton(
  ...
  Future<void> _verplaats(int oudIndex, int nieuwIndex) async {
    ...
    final lijst = List<OffertePrijsVoorAllePositiesTemplateModel>.from(
      _templates,
    );
    ...
    final hernummerd = List<OffertePrijsVoorAllePositiesTemplateModel>.generate(
      lijst.length,
  ...
  Future<OffertePrijsVoorAllePositiesTemplateModel?> _toonTemplateDialog({
    required OffertePrijsVoorAllePositiesTemplateModel begin,
    required bool nieuw,
  }) async {
    ...
    try {
      return await showDialog<OffertePrijsVoorAllePositiesTemplateModel>(
        context: context,
        builder: (dialogContext) {
          return StatefulBuilder(
            builder: (context, setDialogState) {
              ...
              return AlertDialog(
                ...
                title: Text(
                  nieuw
                      ? 'Prijs voor alle posities toevoegen'
                      : 'Prijs voor alle posities wijzigen',
                ...
                content: SizedBox(
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: <Widget>[
                        const Text(
                          'Deze regel is een herbruikbaar sjabloon. De prijs zelf wordt later '
                          'in de offerte ingevuld en op de gekozen posities toegepast. '
                          'Na overnemen is de regel onafhankelijk van deze bibliotheek.',
                          style: TextStyle(
                            color: _tekstGrijs,
                            fontSize: 12.3,
                            height: 1.4,
...
class _TemplateKaart extends StatelessWidget {
  ...
  final OffertePrijsVoorAllePositiesTemplateModel template;
